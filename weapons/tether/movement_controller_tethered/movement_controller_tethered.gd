class_name Movement_Controller_Tethered
extends Movement_Controller_Base


static func construct() -> Movement_Controller_Tethered:
	return preload("Movement_Controller_Tethered.tscn").instantiate();


func start(body : Node3D):
	super(body);
	assert(body is CharacterBody3D, "Movement_Controller_Tethered must be applied to a CharacterBody3D");
	
	for movement_state in $State_Machine.get_children():
		movement_state.start(body);
	
	# Determine initial state
	if !body.is_on_floor(): $State_Machine.current_state = $State_Machine/Tethered_Air_State;
	else:
		if body.velocity.is_zero_approx(): $State_Machine.current_state = $State_Machine/Tethered_Idle_State;
		else: $State_Machine.current_state = $State_Machine/Tethered_Walk_State;
